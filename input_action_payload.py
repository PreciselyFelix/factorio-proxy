...
class CheckCRCHeuristicPayload(InputActionPayload):
    ...
    @classmethod
    def from_bitstream(cls, bit_stream: BitStream):
        player_index, crc, tick_of_crc = bit_stream.readlist("uintle8, uintle32, uintle32")

        return cls(player_index, crc, tick_of_crc)
    
    def to_bitstream(self) -> BitStream:
        return_stream = BitStream(f"uintle8={self.player_index}, uintle32={self.crc}, uintle32={self.tick_of_crc}")
...
class StartWalkingPayload(InputActionPayload):
    ...
    def to_bitstream(self) -> BitStream:
        return_stream = BitStream(f"uintle8={self.player_index}, uintle8={self.direction.value}")
        return return_stream
